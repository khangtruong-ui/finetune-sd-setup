...
def save_checkpoint(config, epoch, state, text_encoder_params, vae_params, tokenizer, text_encoder, vae, unet):
    ...
    subprocess.run(f'gsutil -m cp -r ./sd-full-finetuned/* {gs_directory}/full > upload.log &', shell=True)
    subprocess.run(f'gsutil -m cp *.log {gs_directory}/log >> upload.log &', shell=True)
